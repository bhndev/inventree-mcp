...
- **Bearer writes are CSRF-rejected, and `client.go` works around it.** Measured against 1.4.0 (API 511): with an OAuth bearer token every read succeeds and every write returns 403 `{"detail":"CSRF Failed: Referer checking failed - no Referer."}`. The same payload with `Authorization: Token` returns 201. Cause, in InvenTree's `settings.py`: `DEFAULT_AUTHENTICATION_CLASSES` lists `SessionAuthentication` *before* `ExtendedOAuth2Authentication`, and `oauth2_provider.middleware.OAuth2TokenMiddleware` sets `request.user` from the bearer token before DRF runs — so `SessionAuthentication` sees an active user, calls `enforce_csrf()`, and the OAuth authenticator is never reached. `ApiTokenAuthentication` is first in the list, which is why `Token` auth is unaffected; GET is CSRF-exempt, which is why reads work. **This mimics a permissions problem exactly** — check the response *body* before touching scopes or roles.

  `setCSRFHeaders` in `internal/client/client.go` satisfies the check on bearer writes by sending `Origin`, a `csrftoken` cookie, and a matching `X-CSRFToken`. Django's CSRF is double-submit (`CSRF_USE_SESSIONS` is unset, so the secret comes from the cookie and is compared only against the header), and `_origin_verified` short-circuits when `Origin` equals the request's own host. This is not a bypass of anything real: CSRF defends *ambient* credentials, and this client sends no session cookie. Do not extend it to `Token` auth or to safe methods — that would imply a problem that isn't there. Confirm with a `POST /api/part/` carrying `{}`: 403 CSRF means blocked, 400 `{"name":["This field is required."]}` means auth passed and nothing was created.

- **`REST_FRAMEWORK` is not configurable, `MIDDLEWARE` is.** Fixing the ordering upstream is the real repair, but `REST_FRAMEWORK` is a hardcoded dict. `MIDDLEWARE = CONFIG.get('middleware', [...])` *is* overridable from `config.yaml`, and `OAuth2TokenMiddleware` appears exactly once in the whole codebase, so removing it there also fixes this — at the cost of pinning the entire 23-entry list against upgrades. The key is undocumented and absent from `config_template.yaml`.

- **Client-credentials tokens cannot call the API.** `oauth2_validators.py` sets `request.user = None` for that grant, so DRF's `IsAuthenticated` rejects every call and it looks like a bad token. To mint a token for testing, use the password, device-code, or implicit grant — or, on DOT 3.3.0 (what 1.4.0 pins), create the `AccessToken` row directly in the Django admin: `AccessTokenAdmin` is a plain `ModelAdmin` there, and `TokenChecksumField.pre_save` derives the checksum from whatever token string is entered.
