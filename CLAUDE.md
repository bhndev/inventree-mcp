...
- **Several write operations are performed by a background worker**, so a 200 means "accepted", not "done", and the next step in the workflow can fail if called immediately. Confirmed for `auto-allocate/`, `complete/` (outputs) and sales order shipping. The symptom is a validation error naming work you just did — `Required stock has not been fully allocated`, `Required build quantity has not been completed`, `Order has incomplete line items`. The fix is to re-read and retry, **not** to set an `accept_*` override, which would close the order short instead.
